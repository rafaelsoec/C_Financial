...
void executeMartingale(PositionInfo& position){
   if (ENABLE_MARTINGALE) {
      double points = MARTINGALE_POINTS; 
      double newVolume = position.volume;
      double counter = points; 
...
void executePatterns(MainCandles& mainCandles){
   ...
   if (secLastBodyPoints >= MIN_BODY_POINTS && thirdLastBodyPoints >= MIN_BODY_POINTS  ) {
      ...
      double secureProportion = 0.6;
      
      double closeSpread = type == BUY ? calcPrice(actualCandle.close,  mainCandles.spread) : calcPrice(actualCandle.close, -mainCandles.spread);
      ...
      if (mainCandles.secondLastOrientation != mainCandles.thirdLastOrientation && mainCandles.secondLastOrientation != mainCandles.lastOrientation) {
         if ((possuiCorpoProporcional(secLastCandle, thirdLastCandle, 20) && possuiCorpoProporcional(secLastCandle, lastCandle, 20)) && waitCandlesP1 <= 0) {
            ...
            }else if ((mainCandles.actualOrientation == DOWN && mainCandles.lastOrientation == DOWN && closeSpread < lastCandle.close)){
               double sl = calcPoints(lastCandle.high, actualCandle.close);
               double tp = sl;
   ...
   /* */
      // Separado faz 66%
     if (mainCandles.secondLastOrientation == mainCandles.thirdLastOrientation && mainCandles.secondLastOrientation != mainCandles.lastOrientation) {
         if (secLastBodyPoints < thirdLastBodyPoints && secLastBodyPoints < lastBodyPoints && waitCandlesP3 <= 0) {
            double tp = calcPoints(thirdLastCandle.open, actualCandle.close);
            double sl = calcPoints(secLastCandle.open, actualCandle.close);
            if (sl < tp && tp > secLastBodyPoints) {
               if (mainCandles.actualOrientation == DOWN && mainCandles.lastOrientation == DOWN && closeSpread < secLastCandle.open){
                  PositionInfo position = realizeDeals(type, activeVolume, sl, tp , magicNumber);
                  drawHorizontalLine(actualCandle.close, 0, HORIZONTAL_LINE + "_Pattern3_" + TimeToString(TimeCurrent()), indColor);
                  patterns[1] += 1;
                  waitCandlesP3 = 1;
               }else if (mainCandles.actualOrientation == UP && mainCandles.lastOrientation == UP && closeSpread > secLastCandle.open){
                  PositionInfo position = realizeDeals(type, activeVolume, sl, tp , magicNumber);
                  drawHorizontalLine(actualCandle.close, 0, HORIZONTAL_LINE + "_Pattern3_" + TimeToString(TimeCurrent()), indColor);
                  patterns[1] += 1;
                  waitCandlesP3 = 1;
         ...
         else if (secLastBodyPoints > thirdLastBodyPoints && secLastBodyPoints > lastBodyPoints && lastBodyPoints > secLastBodyPoints * secureProportion && waitCandlesP4 <= 0) {
            double sl = calcPoints(secLastCandle.close, actualCandle.close);
            double tp = sl;
            if (sl < tp && tp > thirdLastBodyPoints) {
               if (mainCandles.actualOrientation == DOWN && mainCandles.lastOrientation == DOWN && closeSpread < secLastCandle.close * secureProportion){
                  PositionInfo position = realizeDeals(type, activeVolume, sl, tp , magicNumber);
                  drawHorizontalLine(actualCandle.close, 0, HORIZONTAL_LINE + "_Pattern4_" + TimeToString(TimeCurrent()), indColor);
                  patterns[1] += 1;
                  waitCandlesP4 = 1;
               }else if (mainCandles.actualOrientation == UP && mainCandles.lastOrientation == UP && closeSpread > secLastCandle.close * secureProportion){
                  PositionInfo position = realizeDeals(type, activeVolume, sl, tp , magicNumber);
                  drawHorizontalLine(actualCandle.close, 0, HORIZONTAL_LINE + "_Pattern4_" + TimeToString(TimeCurrent()), indColor);
                  patterns[1] += 1;
                  waitCandlesP4 = 1;
...
double getAverageLastCandles(int quantidade) {
   if(quantidade <= 0) {
      return 0.0;
   }
   ...
   int copiados = CopyRates(_Symbol, PERIOD, 1, quantidade, rates);

   if(copiados <= 0){
      Print("Erro ao copiar candles. GetLastError() = ", GetLastError());
      return 0.0;
   }

   double somaCorpos = 0.0;
   for(int i = 0; i < copiados; i++){
      double corpo = calcPoints(rates[i].close, rates[i].open);
      somaCorpos += corpo;
   }

   return somaCorpos / copiados;
}
...
void moveAutomaticStopPerPoint(){
   ...
   //double points = getAverageLastCandles(200); 
   for(int position = PositionsTotal(); position >= 0; position--)  {
      if(hasPositionOpen(position)){
         ulong ticket = PositionGetTicket(position);
         PositionSelectByTicket(ticket);
