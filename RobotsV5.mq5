...
double VALUE_MOVING_AVERAGE[3], VALUE_MOVING_AVERAGE21[3], VALUE_ADX[3], VALUE_CCI[3], VALUE_ATR[3], PROPORTION_TAKE_STOP = 2;
...
void OnTick() {
   ...
   for(int i = 0; i < ArraySize(configs); i++)  {
      if(!GetLastClosedCandles(configs[i].tf, candles)) {
         printf("Candles Recuperados - " + EnumToString(configs[i].tf));
         return;
      } 
      
      if (!GetMovingAverage(configs[i].tf, 50 , VALUE_MOVING_AVERAGE)) {
         printf("Media 50 Recuperada - " + EnumToString(configs[i].tf));
         return;
      }   
      
      if (!GetAdx(configs[i].tf)) {
         printf("ADX Recuperado - " + EnumToString(configs[i].tf));
         return;
      }   
      
      if (!GetAtr(configs[i].tf)) {
         printf("Atr Recuperado - " + EnumToString(configs[i].tf));
         return;
      }
      ...
      if(IsNewBar(configs[i])) {
         printf("Iniciando validação por novo candle - " + EnumToString(configs[i].tf));
         configs[i].waitNewCandle = false;
         ...
         if (!IsTrendNotExtrapolated(candles[0].close)) {
            printf("Indicadores Extrapolados - " + EnumToString(configs[i].tf));
            return;
         }    
...
bool GetAtr(ENUM_TIMEFRAMES tf) {   
   // Handle MA
   int atrHandle = iATR(_Symbol, tf, 14);
   if(atrHandle == INVALID_HANDLE)
      return false;
      
   if(CopyBuffer(atrHandle, 0, 0, 3, VALUE_ATR) <= 0)
      return false;
...
bool IsTrendNotExtrapolated(double precoAtual, double maxAdx = 45.0) {
   // ADX muito alto -> tendência pode estar esgotada
   if(VALUE_ADX[0] > maxAdx)
      return false;
      
   // Distância entre preço e média
   double distanciaMedia = MathAbs(precoAtual - VALUE_MOVING_AVERAGE[0]);

   double atr = VALUE_ATR[0];
   // Distância máxima permitida baseada no ATR
   double distanciaMinima = atr * ATR_MINIMUM;
   double distanciaMaxima = atr * 30;

   // Preço muito longe da média
   if(distanciaMedia < distanciaMinima)
      return false;
      
   if(distanciaMedia > distanciaMaxima)
      return false;
...
void MoveStopByATR(double multiplicador = 2.0){
   double atr = VALUE_ATR[0];
   for(int i = 0; i < PositionsTotal(); i++) {
      ulong ticket = PositionGetTicket(i);
      if(ticket == 0)
 ...
 //  bool isMercadoNaoLateralizado = DISABLED_LATERAL_MARKET_VALIDATIONS || !MercadoLateral();
   ...
   if(initialTendency == -1  && diff){
      ...
      if( VALUE_MOVING_AVERAGE[0] < candles[4].close && VALUE_MOVING_AVERAGE[0] < candles[3].close && VALUE_MOVING_AVERAGE[0] < candles[2].close 
         ...
         && VALUE_ADX[1] < VALUE_ADX[2] && diff && ultimosCorposCompra > actualBody) {
        ...
        /* if (diff < 300) {
         ...
         trade.SetExpertMagicNumber(config.magicNumber);
         if (!DISABLED_NEGOTIATIONS) {
            bool ok = trade.Sell(newVolume, _Symbol, precoAtual, sl, tp, "SELL_TENDENCY_RMPIMENTO_MEDIA_" + config.label);
            if(ok){
      ...
      if( VALUE_MOVING_AVERAGE[0] > candles[4].close && VALUE_MOVING_AVERAGE[0] > candles[3].close && VALUE_MOVING_AVERAGE[0] > candles[2].close 
         && VALUE_MOVING_AVERAGE[0] < precoAtual && VALUE_MOVING_AVERAGE[1] < precoAtual && VALUE_MOVING_AVERAGE[2] < precoAtual
         && VALUE_ADX[1] > VALUE_ADX[2] && diff && ultimosCorposVenda > actualBody) {
         ...
         trade.SetExpertMagicNumber(config.magicNumber);
         if (!DISABLED_NEGOTIATIONS) {
            bool ok = trade.Buy(newVolume, _Symbol, precoAtual, sl, tp, "BUY_TENDENCY_RMPIMENTO_MEDIA_" + config.label);
            if(ok){
               config.waitNewCandle = true;
               config.maxRobots = NUMBER_MAX_ROBOT_BY_TIMEFRAME;
