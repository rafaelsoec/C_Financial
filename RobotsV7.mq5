...
void VerifyTendency(TimeframeConfig &config) {
 ...
 //  bool isMercadoNaoLateralizado = DISABLED_LATERAL_MARKET_VALIDATIONS || !MercadoLateral(config);
   ...
   if(initialTendency == -1  && diff){
      ...
      if( config.average50[0] < candles[4].close && config.average50[0] < candles[3].close && config.average50[0] < candles[2].close 
         && config.average50[0] > precoAtual && config.average50[1] > precoAtual && config.average50[2] > precoAtual
         && config.adx[1] < config.adx[2] && diff && ultimosCorposCompra > actualBody) {
        ...
        /* if (diff < 300) {
         ...
         trade.SetExpertMagicNumber(config.magicNumber);
         if (!DISABLED_NEGOTIATIONS) {
            bool ok = trade.Sell(NormalizeDouble(newVolume * tendenciaExtrapolada, _Digits), _Symbol, precoAtual, sl, tp, "SELL_TENDENCY_RMPIMENTO_MEDIA_" + config.label);
            if(ok){
               config.waitNewCandle = true;
               config.maxRobots = NUMBER_MAX_ROBOT_BY_TIMEFRAME;
               drawVerticalLine(actualTime, "Object_line_candleCandidato_" + EnumToString(config.tf) + "_SELL_RMPIMENTO_MEDIA" + TimeToString(expiration), clrRed);
      ...
      if( config.average50[0] > candles[4].close && config.average50[0] > candles[3].close && config.average50[0] > candles[2].close 
         && config.average50[0] < precoAtual && config.average50[1] < precoAtual && config.average50[2] < precoAtual
         && config.adx[1] > config.adx[2] && diff && ultimosCorposVenda > actualBody) {
         ...
         double tendenciaExtrapolada = IsTrendSaturated(config, SELL, precoAtual);
         if (!DISABLED_NEGOTIATIONS) {
            bool ok = trade.Buy(NormalizeDouble(newVolume * tendenciaExtrapolada, _Digits), _Symbol, precoAtual, sl, tp, "BUY_TENDENCY_RMPIMENTO_MEDIA_" + config.label);
            if(ok){
               config.waitNewCandle = true;
               config.maxRobots = NUMBER_MAX_ROBOT_BY_TIMEFRAME;
               drawVerticalLine(actualTime, "Object_line_candleCandidato_" + EnumToString(config.tf) + "_BUY_TRUNFO_RMPIMENTO_MEDIA" + TimeToString(expiration), clrGreen);
               Print("BUY TENDENCY executado com sucesso em ", config.label);
            }
         }
      }
