...
input double MOVE_STOP_PROTECTION_PERCENTUAL = 40;
...
void OnTick() {
   ...
   for(int i = 0; i < ArraySize(configs); i++)  {
      ...
      if (!GetMovingAverage(configs[i], MOVING_AVERAGE, configs[i].average50)) {
...
void MoveStopPorPontos()
{
   ...
   for(int i = 0; i < PositionsTotal(); i++) {
      ...
      double profitDiario = AccountInfoDouble(ACCOUNT_PROFIT);
      if (profitDiario >= LOSS_PER_DAY ) {
         double pontosEntry = calcPoints(entry, currentPrice);
         if(type == POSITION_TYPE_BUY && entry > slAtual ) {
            novoSL = entry + (pontosEntry *  percentualMoveStop / 100 * point);
            if(trade.PositionModify(ticket, novoSL, tpAtual)){
               Print("Stop movido - Protecao Por Profit > Loss diario - ", entry, " - Buy");
               return;
            }
         }
         if(type == POSITION_TYPE_SELL && entry < slAtual) {
            novoSL = entry - (pontosEntry *  percentualMoveStop / 100 * point);
            if(trade.PositionModify(ticket, novoSL, tpAtual)) {
               Print("Stop movido - Protecao Por Profit > Loss diario - ", entry, " - Sell");
               return;
            }
         }
      }
...
void VerifyOther(TimeframeConfig &config) {
   if (config.waitNewCandleHighRisk) {
      return;
   }
   
   double pointsLateral = isLateralizado(1, QTD_CANDLES);
   if (pointsLateral == 0) {
      return;
   }
   ...
   double actualPoints = calcPoints(candles[0].open, precoAtual);
   double lastPoints = calcPoints(candles[1].open, candles[1].close);
   
   bool active = false;
   if (lastPoints >= 2 * pointsLateral) {
      double sl = candles[1].open;
      double points = calcPoints(sl, precoAtual) * PROPORTION_TAKE_STOP;
      ...
      if (c2Bear && config.average50[0] > precoAtual && config.average50[1] > precoAtual && config.average50[2] > precoAtual ) {
         drawVerticalLine(candles[0].time, "Object_line_candleCandidato_" + EnumToString(config.tf) + "_SELL_LATERAL" + FormatDateToString(candles[0].time), clrRed);
         if (!DISABLED_NEGOTIATIONS) {
            trade.SetExpertMagicNumber(config.magicNumber);
            double tp = calcPrice(precoAtual, -points);
            newVolume = NormalizeVolume(NormalizeDouble(newVolume * tendenciaExtrapolada, _Digits));
            bool ok = trade.Sell(newVolume, _Symbol, precoAtual, sl, tp, "_SELL_LATERAL_" + config.label);
            if(ok){
               config.waitNewCandleHighRisk = true;
               Print("SELL LATERAL executado com sucesso em ", config.label);
            }
         }
      } else if (c2Bull && config.average50[0] < precoAtual && config.average50[1] < precoAtual && config.average50[2] < precoAtual ) {
         drawVerticalLine(candles[0].time, "Object_line_candleCandidato_" + EnumToString(config.tf) + "_BUY_LATERAL" + FormatDateToString(candles[0].time), clrWhite);
         if (!DISABLED_NEGOTIATIONS) {
            trade.SetExpertMagicNumber(config.magicNumber);
            double tp = calcPrice(precoAtual, points);
            newVolume = NormalizeVolume(NormalizeDouble(newVolume * tendenciaExtrapolada, _Digits));
            bool ok = trade.Buy(newVolume, _Symbol, precoAtual, sl, tp, "_BUY_LATERAL_" + config.label);
            if(ok){
               config.waitNewCandleHighRisk = true;
               Print("BUY LATERAL executado com sucesso em ", config.label);
            }
         }
      }
   }
...
void VerifyEngolfo(TimeframeConfig &config) {
   ...
   if (initialTendency == -1 && c3Bear && c4Bear && c2Bull) {
      ...
      active = true;
   } else if(initialTendency == 1 && c3Bull && c4Bull && c2Bear) {
...
bool isAverageTendency(TimeframeConfig &config, TYPE_NEGOCIATION type, int qtdItems, double precoAtual) {
   ...
   for (int i = 1; i < qtdItems; i++) {
      if (config.average50[i] < precoAtual && config.average50[i-1] > config.average50[i]) {
         buy++; 
      }
...
bool IsMA50Consolidated(TimeframeConfig &config) {
   double slope = MathAbs(config.average50[0] - config.average50[10]);

   double maxMA = config.average50[0];
   double minMA = config.average50[0];

   for(int i = 0; i < 15; i++) {
      maxMA = MathMax(maxMA, config.average50[i]);
      minMA = MathMin(minMA, config.average50[i]);
   }
