...
void VerifyShortTendency(TimeframeConfig &config) {
   ...
   if (IsBullish(candles[2]) && IsBullish(candles[1]) && IsBullish(candles[3])) {
      config.actualTendency = BUY;
   } else if (IsBearish(candles[2]) && IsBearish(candles[1]) && IsBearish(candles[3])) {
      config.actualTendency = SELL;
   }  else {
      config.actualTendency = NONE;
   }
   
   if(config.actualTendency == SELL && IsBiggerBodyThanWick(candles[1], ACCEPTABLE_CANDLE_BODY_PERCENTUAL)){
   ...
   } else  if(config.actualTendency == BUY && IsBiggerBodyThanWick(candles[1], ACCEPTABLE_CANDLE_BODY_PERCENTUAL)){
