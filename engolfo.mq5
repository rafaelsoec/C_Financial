...
int OnInit() {
   ...
   BALANCE_DAILY = AccountInfoDouble(ACCOUNT_BALANCE);
...
void OnTick() {
   ...
   if (hasNewCandle(PERIOD_MULTIPLE_POSITIONS_BY_CANDLE)) {
      ...
      /*
       ...
       if (diff < 0 && MathAbs(diff) > balance * 0.1 * activeVolume) {
    ...
    if (IsNewDay()){ 
      ...
      BALANCE_DAILY = BALANCE;
      MIN_BODY_POINTS = getAverageLastCandles(100);
...
void executePatterns(MainCandles& mainCandles){
   ...
   if (secLastBodyPoints >= MIN_BODY_POINTS && thirdLastBodyPoints >= MIN_BODY_POINTS ) {
      ...
      double proporcao = (ENABLE_MOVESTOP ? PROPORTION_TAKE_STOP : 1);
...
void showComments(){
   ...
   Comment(
         ...
         " Inversão Ativa: ", ENABLE_INVERSION_POSITION,
...
double executeCCI(MainCandles& mainCandles){
   if(CopyBuffer(handleICCI,0,0,periodAval,CCI) == periodAval) {
      double actualIdx = CCI[periodAval-1];
      double lastIdx = CCI[periodAval-2];
      double secondLastIdx = CCI[periodAval-3];
      
      //return (actualIdx + secondLastIdx + lastIdx) / 3;
      return actualIdx;
   }
...
bool hasNewCandle(ENUM_TIMEFRAMES period = PERIOD_CURRENT){
