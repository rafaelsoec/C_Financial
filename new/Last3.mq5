...
struct TimeframeConfig
{
   ...
   int maxRobotsReversionTendency;
   int maxRobotsRompimentoTendency;
   int maxRobotsEngolfo;
   int maxRobotsHighRisk;
   double multiplier;
   datetime lastBarTime;
   bool waitNewCandle;
   bool waitNewCandleEngolfo;
   bool waitNewCandleHighRisk;
   bool waitNewCandleHighRiskCross;
   bool waitNewCandleHighRiskAverage;
   bool waitNewCandleHighRiskReversion;
   bool waitNewCandleHighRiskRompimento;
   bool enableTendency;
...
input  bool ENABLE_SHORT_TENDENCY = true;
 ...
 input bool ENABLE_ROMPIMENTO_EXTREMOS = true;
...
string getContadores(){
   string tendency = "";
   for(int i = 0; i < ArraySize(COUNT_NEGOTIATIONS); i++) {
      if (i == 0) {
         tendency += "Tendency: " +  IntegerToString(COUNT_NEGOTIATIONS[i]) + ", ";
      }
      if (i == 1) {
         tendency += "Engolfo: " + IntegerToString(COUNT_NEGOTIATIONS[i]) + ", ";
      }
      if (i == 2) {
         tendency += "Short Tendency: " +  IntegerToString(COUNT_NEGOTIATIONS[i]) + ", ";
      }
      if (i == 3) {
         tendency += "Cross Tendency: " +  IntegerToString(COUNT_NEGOTIATIONS[i]) + ", ";
      }
      if (i == 4) {
         tendency += "Average Tendency: " + IntegerToString(COUNT_NEGOTIATIONS[i]) + ", ";
      }
      if (i == 5) {
         tendency += "Rompimento Tendency: " + IntegerToString(COUNT_NEGOTIATIONS[i]) + ", ";
      }
   }
...
int OnInit()
{
   ...
   for(int i = 0; i < ArraySize(tfs); i++) {
      ...
      configs[i].maxRobotsReversionTendency = NUMBER_MAX_ROBOT;
      configs[i].maxRobotsRompimentoTendency = NUMBER_MAX_ROBOT;
      ...
      configs[i].waitNewCandleHighRiskReversion = false;
      configs[i].waitNewCandleHighRiskRompimento = false;
...
void OnTick() {
  ...
  if(HasNewCandle(PERIOD_M1)) {  
   ...
   for(int i = 0; i < ArraySize(configs); i++)  {
      ...
      resetUnitRobots(configs[i].maxRobotsEngolfo);
      resetUnitRobots(configs[i].maxRobotsTendency);
      ...
      resetUnitRobots(configs[i].maxRobotsReversionTendency);
      resetUnitRobots(configs[i].maxRobotsRompimentoTendency);
      ...
      if (ENABLE_ROMPIMENTO_EXTREMOS) {
          VerifyRompimento(configs[i]);
      }
      ...
      if (ENABLE_AVERAGE_TENDENCY ) {
           VerifyAverage(configs[i]);
      }
      
      if (ENABLE_CROSS_TENDENCY ) {
           VerifyCruzamento(configs[i]);
      }
...
bool IdentificarRompimentos(TimeframeConfig &config, TypeNegotiation type, double precoAtual) {
   if (type == NONE) {
      return false;
   }

   RegiaoExtremo regioes[];
   double precos[];
   double tolerancia = 300;
   int qtdMaxCandles = 200;
   int minToques = 3; 
   int lados = 2; 
   bool valid = false;
   
   bool isTopo = type == BUY;
   if (isTopo && config.movingAverage[0] < precoAtual  && config.movingAverage21[0] < precoAtual ) {
      valid = true;
   } else if (!isTopo  && config.movingAverage[0] > precoAtual && config.movingAverage21[0] > precoAtual) {
      valid = true;
   }
   
   EncontrarRegioesExtremos(_Symbol, config.tf, qtdMaxCandles, lados, tolerancia,  isTopo, regioes);
   return ObterPrecosRegioesAtingidas(regioes, precos, minToques, precoAtual) > 0 && valid;
}

int ObterPrecosRegioesAtingidas(RegiaoExtremo &regioes[], double &precos[], int minToques, double precoAtual) {
   
   ArrayResize(precos, 0);
   for(int i = 0; i < ArraySize(regioes); i++) {
      // Verifica se o preço atual está dentro da região
      if(regioes[i].quantidade >= minToques && precoAtual >= regioes[i].precoMin && precoAtual <= regioes[i].precoMax) {
         int tamanho = ArraySize(precos);

         ArrayResize(precos, tamanho + 1);

         precos[tamanho] = regioes[i].precoMedio;
      }
   }

   return ArraySize(precos);
}
...
bool IsMaxRobots() {
   ...
   return PositionsTotal() > NUMBER_MAX_ROBOT * count;// * ArraySize(tfs);
}
   ...
   for(int i = 0; i < total; i++) {
      ...
      if (profit > 0) {
         ...
         if(type == POSITION_TYPE_BUY ) {
            ...
            if(ENABLE_MULTI_ROBOTS_IN_PROFIT && isEnableMultiProfit) {
...
VolumeLevel GetVolumeLevel(ENUM_TIMEFRAMES timeframe, bool useRealVolume = false) {
   ...
   int period = 20;
   double lowFactor = 0.7;
   double highFactor = 1.3;
   
   // Calcula a média dos últimos N candles (exceto o atual)
   for(int i = 1; i <= period; i++)
   {
      if(useRealVolume)
         avgVolume += (double)iVolume(symbol, timeframe, i);
      else
         avgVolume += (double)iTickVolume(symbol, timeframe, i);
   }
   ...
   if(useRealVolume)
      currentVolume = (double)iVolume(symbol, timeframe, 0);
   else
      currentVolume = (double)iTickVolume(symbol, timeframe, 0);

   if(currentVolume < avgVolume * lowFactor)
      return VOLUME_LOW;

   if(currentVolume > avgVolume * highFactor)
      return VOLUME_HIGH;
...
LOSS_TREND GetLossTrend(LossTrade &losses[]) {
   int buyLoss=0;
   int sellLoss=0;

   for(int i=0;i<ArraySize(losses);i++) {
      if(losses[i].type==DEAL_TYPE_BUY)
         buyLoss++;

      if(losses[i].type==DEAL_TYPE_SELL)
         sellLoss++;
   }
   ...
   if(buyLoss > sellLoss*1.5)
      return LOSS_BUY;

   if(sellLoss > buyLoss*1.5)
      return LOSS_SELL;
...
bool GetLastHourLosses(string symbol, LossTrade &losses[], int qtdLosses) {
   ...
   for(int i=0;i<total;i++) {
      ...
      int pos = ArraySize(losses);
      ArrayResize(losses,pos+1);

      losses[pos].closeTime = (datetime)HistoryDealGetInteger(ticket,DEAL_TIME);
      losses[pos].profit    = profit;
      losses[pos].type      = (ENUM_DEAL_TYPE)HistoryDealGetInteger(ticket,DEAL_TYPE);
      losses[pos].price     = HistoryDealGetDouble(ticket,DEAL_PRICE);
   }

   return ArraySize(losses) > qtdLosses;
}
...
void VerifyReversalTendency(TimeframeConfig &config) {
   ...
   if(config.maxRobotsReversionTendency < 0) {
     return;
   }

   if (config.waitNewCandleHighRiskReversion) {
     return;
   }
   ...
   if (DetectReversal(config, SELL)) {
      ...
      if (!DISABLED_NEGOTIATIONS) {
         ...
         if(ok){
            ...
            config.waitNewCandleHighRiskReversion = true;
            config.maxRobotsReversionTendency--;
         }
      }
...
void VerifyEngolfo(TimeframeConfig &config) {
    ...
    if(config.waitNewCandleEngolfo) {
      return;
    }
...
void VerifyAverage(TimeframeConfig &config) {
   ...
   if (config.waitNewCandleHighRiskAverage) {
      return;
   }
   
   if (IsBearish(candles[1]) 
      && IsBearish(candles[0]) 
      && config.movingAverage[2] > precoAtual 
      && config.movingAverage21[0] > precoAtual) {
      config.actualTendency = SELL;
      ...
      if (!DISABLED_NEGOTIATIONS) {
         ...
         newVolume = NormalizeVolume(NormalizeDouble(newVolume * tendenciaExtrapolada, _Digits));
         bool ok = trade.Sell(newVolume, _Symbol, precoAtual, sl, tp, "SELL_AVERAGE_TENDENCY_" + config.label);
         if(ok){
            COUNT_NEGOTIATIONS[4] += 1;
            ExecuteMartingale(config, SELL, calcPoints(candles[2].open, candles[1].close), precoAtual, newVolume, sl, tp, 0.15);
            drawVerticalLine(actualTime, "Object_line_candleCandidato_" + EnumToString(config.tf) + "_SELL_AVERAGE_CANDIDATO" +  FormatDateToString(candles[0].time), clrRosyBrown);
            Print("SELL AVERAGE TENDENCY executado com sucesso em ", config.label);
            config.waitNewCandleHighRiskAverage = true;
            config.maxRobotsAverageTendency--;
         }
      }
   } else if (IsBullish(candles[1]) 
      && IsBullish(candles[0]) 
      && config.movingAverage[0] < precoAtual 
      && config.movingAverage21[0] < precoAtual) {
      config.actualTendency = BUY;
      ...
      if (!DISABLED_NEGOTIATIONS ) {
         ...
         newVolume = NormalizeVolume(NormalizeDouble(newVolume * tendenciaExtrapolada, _Digits));
         bool ok = trade.Buy(newVolume, _Symbol, precoAtual, sl, tp, "BUY_AVERAGE_TENDENCY_" + config.label);
         if(ok){
            COUNT_NEGOTIATIONS[4] += 1;
            ExecuteMartingale(config, BUY, calcPoints(candles[2].open, candles[1].close), precoAtual, newVolume, sl, tp, 0.15);
            drawVerticalLine(actualTime, "Object_line_candleCandidato_" + EnumToString(config.tf) + "_BUY_AVERAGE_CANDIDATO" +  FormatDateToString(candles[0].time), clrAqua);
            Print("BUY AVERAGE TENDENCY executado com sucesso em ", config.label);
            config.waitNewCandleHighRiskAverage = true;
            config.maxRobotsAverageTendency--;
         }
      }
...
void VerifyRompimento(TimeframeConfig &config) {
   double precoAtual = candles[0].close;
   double newVolume = NormalizeVolume(ENABLE_TIMEFRAME_MULTIPLIER ? VOLUME * config.multiplier : VOLUME);
   bool diff = MathAbs(config.adxMinus[2] - config.adxPlus[2])  > 10;
   datetime actualTime = TimeCurrent();
    
   if(config.maxRobotsRompimentoTendency < 0) {
      return;
   }
   ...
   if (config.waitNewCandleHighRiskRompimento) {
      return;
   }
   
   bool isRompimentoVenda = IdentificarRompimentos(config, SELL, precoAtual);
   bool isRompimentoCompra = IdentificarRompimentos(config, BUY, precoAtual);
   if ((isRompimentoCompra || !isRompimentoVenda) && IsBullish(candles[0]) ) {
      printf("Rompimento de TOPO");
      config.actualTendency = BUY;
      double sl = candles[2].low;
      double diff = calcPoints(precoAtual, sl) * PROPORTION_TAKE_STOP;
      double tp = NormalizeDouble(calcPrice(precoAtual, diff), _Digits);
      ...
      if (!DISABLED_NEGOTIATIONS ) {
         ...
         newVolume = NormalizeVolume(NormalizeDouble(newVolume * tendenciaExtrapolada, _Digits));
         bool ok = trade.Buy(newVolume, _Symbol, precoAtual, sl, tp, "BUY_ROMPIMENTO_TENDENCY_" + config.label);
         if(ok){
            COUNT_NEGOTIATIONS[5] += 1;
            ExecuteMartingale(config, BUY, calcPoints(candles[2].open, candles[1].close), precoAtual, newVolume, sl, tp, 0.15);
            drawVerticalLine(actualTime, "Object_line_candleCandidato_" + EnumToString(config.tf) + "_BUY_ROMPIMENTO_CANDIDATO" +  FormatDateToString(candles[0].time), clrAqua);
            Print("BUY ROMPIMENTO TENDENCY executado com sucesso em ", config.label);
            config.waitNewCandleHighRiskRompimento = true;
            config.maxRobotsRompimentoTendency--;
         }
      }
   ...
   if ((isRompimentoVenda || !isRompimentoCompra) && IsBearish(candles[0])) {
      printf("Rompimento de FUNDO");
      config.actualTendency = SELL;
      double sl = candles[2].high;
      double diff = calcPoints(precoAtual, sl) * PROPORTION_TAKE_STOP;
      double tp = NormalizeDouble(calcPrice(precoAtual, -diff), _Digits);
      ...
      if (!DISABLED_NEGOTIATIONS) {
         ...
         newVolume = NormalizeVolume(NormalizeDouble(newVolume * tendenciaExtrapolada, _Digits));
         bool ok = trade.Sell(newVolume, _Symbol, precoAtual, sl, tp, "SELL_ROMPIMENTO_TENDENCY_" + config.label);
         if(ok){
            COUNT_NEGOTIATIONS[5] += 1;
            ExecuteMartingale(config, SELL, calcPoints(candles[2].open, candles[1].close), precoAtual, newVolume, sl, tp, 0.15);
            drawVerticalLine(actualTime, "Object_line_candleCandidato_" + EnumToString(config.tf) + "_SELL_ROMPIMENTO_CANDIDATO" +  FormatDateToString(candles[0].time), clrRosyBrown);
            Print("SELL ROMPIMENTO TENDENCY executado com sucesso em ", config.label);
            config.waitNewCandleHighRiskRompimento = true;
            config.maxRobotsRompimentoTendency--;
         }
      }
...
void VerifyCruzamento(TimeframeConfig &config) {
   ...
   if (config.waitNewCandleHighRiskCross) {
      return;
   }
   ...
   if(config.adxMinusTendency != NONE  && config.adxPlusTendency != NONE && config.adxMinusTendency != config.adxPlusTendency) {
      if (IsBearish(candles[0]) 
           ...
           // && config.adxMinus[0] <  config.adx[0]
            ...
            && ((config.movingAverage21[0] > precoAtual && config.movingAverage[0] > precoAtual) || (config.movingAverage50[0] > precoAtual && config.movingAverage[0] > precoAtual))) {
         config.actualTendency = SELL;
         double sl = candles[2].high;
         double diff = calcPoints(precoAtual, sl) * PROPORTION_TAKE_STOP;
         double tp = NormalizeDouble(calcPrice(precoAtual, -diff), _Digits);
         ...
         if (!DISABLED_NEGOTIATIONS) {
            ...
            newVolume = NormalizeVolume(NormalizeDouble(newVolume * tendenciaExtrapolada, _Digits));
            bool ok = trade.Sell(newVolume, _Symbol, precoAtual, sl, tp, "SELL_CRUZAMENTO_TENDENCY_" + config.label);
            if(ok){
               COUNT_NEGOTIATIONS[3] += 1;
               ExecuteMartingale(config, SELL, calcPoints(candles[2].open, candles[1].close), precoAtual, newVolume, sl, tp, 0.15);
               drawVerticalLine(actualTime, "Object_line_candleCandidato_" + EnumToString(config.tf) + "_SELL_CRUZAMENTO_CANDIDATO" +  FormatDateToString(candles[0].time), clrRosyBrown);
               Print("SELL CRUZAMENTO TENDENCY executado com sucesso em ", config.label);
               config.waitNewCandleHighRiskCross = true;
               config.maxRobotsCrossTendency--;
            }
         }
      }
      ...
      if (IsBullish(candles[0]) 
         ...
         if (!DISABLED_NEGOTIATIONS ) {
            ...
            newVolume = NormalizeVolume(NormalizeDouble(newVolume * tendenciaExtrapolada, _Digits));
            bool ok = trade.Buy(newVolume, _Symbol, precoAtual, sl, tp, "BUY_CRUZAMENTO_TENDENCY_" + config.label);
            if(ok){
               COUNT_NEGOTIATIONS[3] += 1;
               ExecuteMartingale(config, BUY, calcPoints(candles[2].open, candles[1].close), precoAtual, newVolume, sl, tp, 0.15);
               drawVerticalLine(actualTime, "Object_line_candleCandidato_" + EnumToString(config.tf) + "_BUY_CRUZAMENTO_CANDIDATO" +  FormatDateToString(candles[0].time), clrAqua);
               Print("BUY CRUZAMENTO TENDENCY executado com sucesso em ", config.label);
               config.waitNewCandleHighRiskCross = true;
               config.maxRobotsCrossTendency--;
            }
         }
...
void VerifyTendency(TimeframeConfig &config) {
   ...
   if(initialTendency == -1  && diff){
      ...
      if (config.movingAverage[1] > precoAtual && config.movingAverage[2] > precoAtual 
         && config.adxMinus[0] > config.adxPlus[0] && !BLOCK_SELLS) {
         config.actualTendency = SELL;
      ...
      if (config.movingAverage[1] < precoAtual && config.movingAverage[2] < precoAtual 
         && config.adxPlus[0] > config.adxMinus[0]  && !BLOCK_BUYS) {
         config.actualTendency = BUY;
         MaximosMinimos maxMin = getMinOrMax(1, QTD_CANDLES);
...
double NormalizeVolume(double volume){
   ...
   return NormalizeDouble(volume, _Digits);
}
...
bool MercadoLateral(TimeframeConfig &config, double minIdx = 20, double limiteInclinacao = 0.0001) {
   ...
   bool adxFraco =  config.adx[0] < minIdx || config.adx[0] < (config.adxPlus[0] + config.adxMinus[0]) / 2;
...
bool DetectReversal(TimeframeConfig &config, TypeNegotiation signal) {
   double close0 = iClose(_Symbol,config.tf, 0);
   double close1 = iClose(_Symbol,config.tf, 1);
   ...
   bool candleReverse = DetectEngulfing(config, signal);

   return touchMA || cross9 || candleReverse;
}

bool DetectEngulfing(TimeframeConfig &config, TypeNegotiation positionType) {
   double open1  = iOpen(_Symbol, config.tf, 1);
   double close1 = iClose(_Symbol, config.tf, 1);

   double open2  = iOpen(_Symbol, config.tf, 2);
   double close2 = iClose(_Symbol, config.tf, 2);
...
int EncontrarRegioesExtremos(
   string simbolo,
   ENUM_TIMEFRAMES timeframe,
   int quantidadeCandles,
   int lado,
   double toleranciaPontos,
   bool topo,
   RegiaoExtremo &regioes[]
)
{
   ArrayResize(regioes, 0);

   int totalBars = Bars(simbolo, timeframe);

   if(totalBars <= quantidadeCandles + lado)
      return 0;

   double ponto = SymbolInfoDouble(simbolo, SYMBOL_POINT);
   double tolerancia = toleranciaPontos * ponto;

   // Percorre os candles
   for(int shift = lado;
       shift < quantidadeCandles &&
       shift < totalBars - lado;
       shift++)
   {
      double preco;
      ...
      if(topo)
      {
         preco = iHigh(simbolo, timeframe, shift);
      ...
      {
         preco = iLow(simbolo, timeframe, shift);
      ...
      // ==========================================================
      // Procurar uma região existente
      // ==========================================================

      bool adicionou = false;

      for(int r = 0; r < ArraySize(regioes); r++)
      {
         double distanciaMin =
            MathAbs(preco - regioes[r].precoMin);

         double distanciaMax =
            MathAbs(preco - regioes[r].precoMax);

         if(distanciaMin <= tolerancia ||
            distanciaMax <= tolerancia)
         {
            // Atualiza limite inferior
            if(preco < regioes[r].precoMin)
               regioes[r].precoMin = preco;

            // Atualiza limite superior
            if(preco > regioes[r].precoMax)
               regioes[r].precoMax = preco;

            // Atualiza preço médio
            regioes[r].precoMedio =
               (regioes[r].precoMin +
                regioes[r].precoMax) / 2.0;

            regioes[r].quantidade++;

            // Candle mais recente
            if(shift < regioes[r].ultimoShift)
            {
               regioes[r].ultimoShift = shift;

               regioes[r].ultimoTempo =
                  iTime(simbolo, timeframe, shift);
            }

            // Candle mais antigo
            if(shift > regioes[r].primeiroShift)
            {
               regioes[r].primeiroShift = shift;

               regioes[r].primeiroTempo =
                  iTime(simbolo, timeframe, shift);
            }
      ...
      if(!adicionou)
      {
         int tamanho = ArraySize(regioes);

         ArrayResize(regioes, tamanho + 1);

         regioes[tamanho].precoMin = preco;
         regioes[tamanho].precoMax = preco;
         regioes[tamanho].precoMedio = preco;

         regioes[tamanho].quantidade = 1;

         regioes[tamanho].primeiroShift = shift;
         regioes[tamanho].ultimoShift = shift;

         regioes[tamanho].primeiroTempo =
            iTime(simbolo, timeframe, shift);

         regioes[tamanho].ultimoTempo =
            iTime(simbolo, timeframe, shift);
      }
   }

   return ArraySize(regioes);
}
