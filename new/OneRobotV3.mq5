...
void OnTick()
{
  ...
  // }
   ...
   for(int i = 0; i < ArraySize(configs); i++)  {
      ...
      if (!GetMovingAverage(configs[i].tf, 50 , VALUE_MOVING_AVERAGE) && !GetMovingAverage(configs[i].tf, 21 , VALUE_MOVING_AVERAGE21))
         return;
         ...
         if(type == POSITION_TYPE_BUY ) {
            ...
            if(tf != PERIOD_MN1) {
               if (GetLastClosedCandles(tf, candles)) {
                  MaximosMinimos maxMix = getMinOrMax(1, QTD_CANDLES + (QTD_CANDLES));
   ...
   if(initialTendency == -1){
      if (VALUE_MOVING_AVERAGE[0] > precoAtual && VALUE_MOVING_AVERAGE[1] > precoAtual && VALUE_MOVING_AVERAGE[2] > precoAtual 
         ...
         if (IsTrendSaturated(SELL, precoAtual, maxMin)) {
          // trade.Sell(newVolume, _Symbol, precoAtual, sl, tp, "SELL_TENDENCY_SATURED_" + config.label);
