//+------------------------------------------------------------------+
//|                             DualCCI_Candle_M15_ATR_Manage.mq5    |
//+------------------------------------------------------------------+
#property strict
...
input double          ATRStopMultiplier  = 1.0;   // stop = 1.0 x ATR
input double          ATRTakeMultiplier  = 2.0;   // alvo = 2.0 x ATR

input bool            UseBreakEven       = true;
input double          BreakEvenATR       = 1.0;   // ativa BE após 1 ATR de lucro
input double          BreakEvenOffsetATR = 0.10;  // trava pequeno lucro

input bool            UseTrailing        = true;
input double          TrailingStartATR   = 1.0;   // trailing só começa após 1 ATR
input double          TrailingATR        = 0.8;   // distância do trailing
...
void CheckEntry()
{
   ...
   if(slowBullCross && fastBullAgree && priceBull && candleBull)
   {
      ...
      if(trade.Buy(Lote, _Symbol, ask, sl, tp, "BUY DualCCI ATR"))
         Print("Compra aberta");
      else
   ...
   if(slowBearCross && fastBearAgree && priceBear && candleBear)
   {
      ...
      if(trade.Sell(Lote, _Symbol, bid, sl, tp, "SELL DualCCI ATR"))
         Print("Venda aberta");
      else
